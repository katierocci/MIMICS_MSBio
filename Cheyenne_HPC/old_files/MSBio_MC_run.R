...
write.csv(data.frame(time=wall_time,
		     cores=no_cores), 
	paste0("Cheyenne_HPC/HPC_output/wall_time_", as.character(MIM_runs), "_", format(Sys.time(), "%Y%m%d_%H%M%S_"),  ".csv"), row.names = F)
...
# Save MC output data
##########################################
saveRDS(MC_MIMICS, paste0("Cheyenne_HPC/HPC_output/MIMICS_MSBio_MC_data-r", as.character(MIM_runs), "_", format(Sys.time(), "%Y%m%d_%H%M%S_"),  ".rds"))
