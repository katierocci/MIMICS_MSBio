---
title: "MSBio MIMICS Litter Decomp Incubation Simulation"
...
#### **Initial modeling goal is to align MIMICS projections with Fig. 2E and the mesic slopes in Fig. 3.**

</br>

</br>

##### Polussa et al. Fig. 2: Litter mineralization rates over 202 days.
<center>
...
##### Polussa et al. Fig. 3: Cumulative mineralization for unique decomposer communities sourced from within the SCBI (a) and HARV (b) sites.
<center>
...
##### Polussa et al. Fig. 1: Soil moisture variation in sampling points from HARV (a) and SCBI (b) from Spring 2020.
<center>
...
* Set ANPP, CLAY and LIG:N input to be equal across all simulations.
* MAT temperature set to equal 20 C, same as the incubation.
* Mean annual soil moisture (MASM) difference between sites guesstimated from difference in field soil GWC.
* Using unmodified field soil GWC 
...
### Important notes and MIMICS code modifications:
...
3. Initiate with litter = 100, SOM pools = 0. MIC pools = 0.01.

4. ANPP value is contributing to Tau modifier, but adding no C input to simulation.

5. Clay impact on simulation?

6. Guesstimate used for litter lignin:N
...
The follwoing code shows how to run a MIMICS INC simulation for a single sample.

```{r echo=TRUE, eval=TRUE, warning=FALSE}
# Example forcing data for a single sample
...
```{r echo=TRUE, eval=TRUE, warning=FALSE}
############
# Plot MIMICS output for a single MIMICS simulation
###########
...
#### First step, create function to load parameter values, run MIMICS-INC ftn, and return summarized MIMICS output

*Function referred to as "MIM-repeat"*
...
#### Example use of MIM-repeat function
...
#### Plot MIM-repeat output

```{r echo=TRUE}
MIMresults <- readRDS(paste0("MCMC/output/MSBIO_INC_MIMresults_", Sys.Date()-1,".rds"))
...
#### Second step, create MCMC function to pick parameter proposals from walking, uniform distributions.

*Referred to as MCMC optimization*

```{r echo=TRUE, eval=FALSE, class.source = 'fold-hide'}
...
#### Plot showing example of MCMC optimization walk
```{r echo=TRUE, class.source = 'fold-hide'}
### Plot MCMC walk

# Load the data
MCMC_out <- readRDS(paste0("MCMC/output/MSBIO_INC_MCMC_out_", Sys.Date()-1,".rds"))
...
#### We're just above the ground floor with this code. 

* Scalar application and equations are an educated, shot in dark.
* Moving to a daily time-step will greatly increase MCMC & MC efficiency.
* Worth solving for core parameters in addition to moisture scalars?
* MCMC targets may need to be adjusted.
* Not yet using MCMC to solve for CO2-C production
* How to use microbial biomass data?
