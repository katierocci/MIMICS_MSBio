...
base <- read.csv("output/LTER_MIMICS_Litter_Decomp_output.csv")
...
adj <- read.csv("output/LTER_MIMICS_Litter_Decomp_output_ADJ.csv")
...
LTER_MAT <- read.csv("LTER_MAT.csv")
colnames(LTER_MAT)[1] <- "site"
plot_df <- left_join(comp_df, LTER_MAT, by=)

# Set SITE order
plot_df <- plot_df %>% filter(site != "LUQ") %>%
  mutate(site = fct_relevel(site, levels = "ARC","BNZ","NWT","HBR","CDR","HFR","AND","SGS","KBS","CWT","KNZ","JRN","SEV"))#,"LUQ"))

png("LIT_Bag_diff.png", width = 2200, height = 1200, res = 300)
ggplot(plot_df, aes(x=site, y=p_diff*100, fill=MAT)) + geom_bar(stat="identity")+
  ...
  labs(title="Parameter adjustment effect on litter bag decomposition rates",
          subtitle ="LTER Sites (n = 14)") +
  xlab("Site") +
  ylab("Change in deocmposition rate (%)") +
  labs(fill="MAT (°C)") +
  scale_y_continuous(limits=c(-50,140), breaks = seq(-60, 140, 20)) +
