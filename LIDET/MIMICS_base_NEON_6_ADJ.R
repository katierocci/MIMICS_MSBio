...
###ADJUSTING VSLOPE AND KINT by MAT
  ...
  Vint_ADJ = Vint * ((-0.0029*TSOI) + 1.0786)
  ...
  Vmax     <- exp(TSOI * Vslope_ADJ + Vint_ADJ) * aV
...
NEON <- read.csv("NEON_SITE_1.csv")
site_no <- 3
Site <- NEON$Site[site_no]
data <- data.frame(Site = NEON$Site[site_no],
                   ANPP = NEON$ANPP[site_no],
                   tsoi = NEON$MAT[site_no],
                   clay = NEON$CLAY2[site_no],
                   lig_N = 0)
...
ggplot(mean_bag_decomp, aes(x=yr, y=meanBAG)) + 
  geom_line(size=1.5) +
  ggtitle(paste("NEON SITE: ", Site, " - Leaf litter decomp")) +
...
#png("NEON_site_LIT_decomp.png", width = 1600, height = 1200, res = 300)
...
#--------------------------------------------------------------
# Collect data for all litter type bags across all NEON sites
#--------------------------------------------------------------
NEON <- read.csv("NEON_SITE_1.csv")
NEON_bag_decomp <- NULL
for(s in 1:nrow(NEON)) {
  site_no <- s
  Site <- NEON$Site[site_no]
  data <- data.frame(Site = NEON$Site[site_no],
                     ANPP = NEON$ANPP[site_no],
  ...
  NEON_bag_decomp <- rbind(NEON_bag_decomp, bag_decomp)
}

#Save the output
write.csv(NEON_bag_decomp, "output/NEON_MIMICS_Litter_Decomp_output_ADJ_3.csv")  
...
#--------------------------------------------------
meanBAGS <- NEON_bag_decomp %>% group_by(site, yr) %>%
              summarize(n = n(),
                        BagLeft_mn = mean(bag),
                        BagLeft_sd = sd(bag))

library(forcats)
meanBAGS<- meanBAGS %>% ungroup %>%
  mutate(site = fct_relevel(site, levels = "TREE","BART","SERC","GRSM","TALL","LENO"))

ggplot(meanBAGS, aes(x=yr, y=BagLeft_mn, color=site)) + 
  geom_line(size=1) +
  scale_color_manual(values=c("#0000cc","#6c34b3", "#ccb909", "#ccb007","#cc4a05","#cb6208")) + 
  ggtitle("NEON SITES: MIMICS mean leaf litter decompostion") +
  ylab("Mass remaining (%)") + xlab("Time (y)") +
  labs(color = "Litter Type") +
  theme_minimal()  
#ggsave('output/NEON_mean_decomp_ADJ.png', dpi=300, width = 5, height = 5)
...
#   labs(color = "Litter Type") +
#   theme_minimal() 
#ggsave('output/NEON_stdev_decomp.tiff', dpi=300, width = 8, height = 5)
